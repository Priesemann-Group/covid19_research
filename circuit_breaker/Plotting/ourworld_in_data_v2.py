...
# @Last Modified: 2020-11-13 22:36:38
# ------------------------------------------------------------------------------ #
# todo:
...
# at most 3% case fatality rate
# at least 10 mio inhabitants
# ------------------------------------------------------------------------------ #
...
# countries with kwargs for plotstyle customization
# c_top = {
#     "United States": {},
#     "Brazil": {},
#     "Colombia": {},
...
c_mid = {
    ...
    "Cyprus" : {},
    "Czech Republic" : {},
    "Denmark" :  {'lw':2, 'zorder':1},
    "Estonia" : {},
    "Finland" :  {},
    "France" : {},
    "Germany" :  {'lw':2, 'zorder':1},
    "Greece" : {},
    "Hungary" : {},
    "Iceland" : {},
    "Ireland" : {},
    ...
    "Norway" :  {},
    "Poland" : {},
    "Portugal" : {'lw':2, 'zorder':1},
    "Romania" : {},
    "Slovakia" : {},
    "Slovenia" : {},
    "Spain" :  {'lw':2, 'zorder':1},
    "Sweden" :  {'lw':2, 'zorder':1},
    "Switzerland" : {},
    "United Kingdom" : {},
}
...
def plot_ts(
    ...
    for idx, c in enumerate(countries.keys()):
        ...
        temp = dat[np.where(np.isfinite(dat))[0]]
        m.append(temp)
        ...
        print(f"{c}: {np.median(temp):.1f} | {deaths/cases*100:.1f} % ")
        df[c] = dat
        ...
        kwargs = countries[c]
        if "lw" not in kwargs:
            kwargs = dict(kwargs, lw=1)
            kwargs = dict(kwargs, alpha=.5)
        if "label" not in kwargs:
            kwargs = dict(kwargs, label=c)
        if "color" not in kwargs:
            kwargs = dict(kwargs, color=clr)

        ax.plot(dat, **kwargs)

    for idx, ts in enumerate(m):
        assert len(ts) == len(m[0]), f"{idx}"
    m = np.array(m)
    mm = np.median(m)
