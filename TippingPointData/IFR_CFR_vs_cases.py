...
def inzidenz_per_lk_and_age():
    ...
    data_l = data_l.rolling(7).sum()

    level_values = data_l.index.get_level_values
    data_l = data_l.groupby(
        [level_values(i) for i in [0,]] + [pd.Grouper(freq="D", level="date")]
    ).sum()
...
if __name__ == "__main__":
    inzidenz = inzidenz_per_lk_and_age()

    fig, axes = plt.subplots(1, 1, figsize=(12, 9))

    for age_group in inzidenz.get_level_values(level="Altersgruppe").unique():
        fig.plot(inzidenz[age_group])
